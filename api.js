...
function SearchPartecipant(summonerGameName, summonerTagLine, summonerPuuid){
  const summonerForm = document.querySelector("#summonerForm");

        summonerForm.puuid.value = summonerPuuid;
        summonerForm.gameName.value = summonerGameName;
        summonerForm.tagLine.value = summonerTagLine;
        summonerForm.submit();
}
...
async function onJsonMatchByID(json){
  ...
  if (json.hasOwnProperty('status')) {
    ...
    if (statusCode === 400) {
        console.log('Si è verificato un errore con codice 400 (not found)');
    } else if (statusCode== 403) {
        console.log('Si è verificato un errore con codice 403 (forbidden): controlla la tua api key!');
    }else if (statusCode== 404) {
      console.log('Si è verificato un errore con codice 404: match file not found. Probabilmente la partita è troppo vecchia ed è stata cancellata dai server della Riot');
    }
    else{
  ...
  for(i=0; i<runesJson.length; i++){
    if(runesJson[i]["id"] == allPlayersInfo[myIndex]["secondaryRuneId"]) {
      secondaryRune_url = "https://ddragon.leagueoflegends.com/cdn/img/" + runesJson[i]["icon"];
    }
    if(runesJson[i]["id"] == allPlayersInfo[myIndex]["primaryRuneId"]){
      const specificRuneArray = runesJson[i]["slots"][0]["runes"];
      for(let j=0; j<specificRuneArray.length; j++){
        if(specificRuneArray[j]["id"] == allPlayersInfo[myIndex]["specificPrimaryRuneId"]) primaryRune_url = "https://ddragon.leagueoflegends.com/cdn/img/" + specificRuneArray[j]["icon"];
      }
    }
  }
  ...
  for(let i=0; i<allPlayersInfo.length; i++){
    ...
    const fullStats = partecipantTeam2.querySelector(".fullStats");

    const player = document.createElement("div");
    player.classList.add("player");

    const nameSumsRunes = document.createElement("div");
    nameSumsRunes.classList.add("nameSumsRunes");

    const champIcon = document.createElement("div");
    champIcon.classList.add("champIcon");
    const span0 = document.createElement("span");
    champIcon.appendChild(span0);

    const summonersIcons = document.createElement("div");
    summonersIcons.classList.add("summonersIcons");
    const sum1 = document.createElement("img");
    sum1.classList.add("sum1");
    const sum2 = document.createElement("img");
    sum2.classList.add("sum2");
    summonersIcons.appendChild(sum1);
    summonersIcons.appendChild(sum2);

    const runesIcons = document.createElement("div");
    runesIcons.classList.add("runesIcons");
    const mainRune = document.createElement("img");
    mainRune.classList.add("mainRune");
    const secondaryRune = document.createElement("img");
    secondaryRune.classList.add("secondaryRune");
    runesIcons.appendChild(mainRune);
    runesIcons.appendChild(secondaryRune);

    const name = document.createElement("div");
    name.classList.add("name");
    const summonerName = document.createElement("span");
    summonerName.classList.add("summonerName");
    name.appendChild(summonerName);

    nameSumsRunes.appendChild(champIcon);
    nameSumsRunes.appendChild(summonersIcons);
    nameSumsRunes.appendChild(runesIcons);
    nameSumsRunes.appendChild(name);

    const opScore = document.createElement("div");
    opScore.classList.add("opScore");

    const Kda = document.createElement("div");
    Kda.classList.add("Kda");
    const UMA = document.createElement("span");
    UMA.classList.add("UMA");
    Kda.appendChild(UMA);
    const kPerc = document.createElement("span");
    kPerc.classList.add("kPerc");
    Kda.appendChild(kPerc);
    const kdaPerc = document.createElement("span");
    kdaPerc.classList.add("kdaPerc");
    Kda.appendChild(kdaPerc);

    const damage = document.createElement("div");
    damage.classList.add("damage");
    const span1 = document.createElement("span");
    const span2 = document.createElement("span");
    damage.appendChild(span1);
    damage.appendChild(span2);
    const dmgDealt = document.createElement("div");
    dmgDealt.classList.add("dmgDealt");
    dmgDealt.classList.add("bar");
    const fillBar = document.createElement("div");
    fillBar.classList.add("fillBar");
    dmgDealt.appendChild(fillBar);
    const dmgTaken = document.createElement("div");
    dmgTaken.classList.add("dmgTaken");
    dmgTaken.classList.add("bar");
    const fillBar2 = document.createElement("div");
    fillBar2.classList.add("fillBar");
    dmgTaken.appendChild(fillBar2);
    damage.appendChild(dmgDealt);
    damage.appendChild(dmgTaken);

    const wards = document.createElement("div");
    wards.classList.add("wards");
    const pink = document.createElement("div");
    pink.classList.add("pink");
    const placedKilled = document.createElement("div");
    placedKilled.classList.add("placedKilled");
    wards.appendChild(pink);
    wards.appendChild(placedKilled);

    const cs = document.createElement("div");
    cs.classList.add("cs");
    const span3 = document.createElement("span");
    const span4 = document.createElement("span");
    cs.appendChild(span3);
    cs.appendChild(span4);
    ...
    for(let i=0; i<7; i++){
      ...
      item0.dataset.num=i;
      div0.appendChild(item0);
      Items.appendChild(div0);
    }
    
    player.appendChild(nameSumsRunes);
    player.appendChild(opScore);
    player.appendChild(Kda);
    player.appendChild(damage);
    player.appendChild(wards);
    player.appendChild(cs);
    player.appendChild(Items);

    fullStats.appendChild(player);
    
    const partecipantDiv2 = player;
  ...
  for(let j=0; j<runesJson.length; j++){
    if(runesJson[j]["id"] == allPlayersInfo[i]["secondaryRuneId"]) {
      partecipantSecondaryRune_url = "https://ddragon.leagueoflegends.com/cdn/img/" + runesJson[j]["icon"];
    }
    if(runesJson[j]["id"] == allPlayersInfo[i]["primaryRuneId"]){
      ...
      for(let k=0; k<specificRuneArray.length; k++){
        if(specificRuneArray[k]["id"] == allPlayersInfo[i]["specificPrimaryRuneId"]) partecipantPrimaryRune_url = "https://ddragon.leagueoflegends.com/cdn/img/" + specificRuneArray[k]["icon"];
      }
    }
  }
    ...
    const partecipantDiv = document.createElement('div');
    partecipantDiv.classList.add('player');

    const img = document.createElement('img');
    const span = document.createElement('span');

    partecipantDiv.appendChild(img);
    partecipantDiv.appendChild(span);
    ...
    partecipantTeam.appendChild(partecipantDiv);
    

    if(queueName === "Arena"){
      const mainRunes = newMatchBox.querySelectorAll(".mainRune");
      const secondaryRunes = newMatchBox.querySelectorAll(".secondaryRune");

      for(rune of mainRunes) rune.remove();
      for(rune of secondaryRunes) rune.remove();
    } 
  }
  ...
  async function getPlayerElo(playerInfo){
    ...
    const response_summoner = await fetch('http://127.0.0.1/summoner_v4.php?puuid=' + playerInfo["puuid"]);
    const summoner_json = await response_summoner.json();

    
    const response_league = await fetch('http://127.0.0.1/league_v4.php?id=' + summoner_json.id).then(onResponse).then(onJsonLeaguev4);
...
function onJsonSummonerv4(json) {
    ...
    fetch('http://127.0.0.1/league_v4.php?id=' + id).then(onResponse).then(onJsonLeaguev4);
...
async function addToFavourite(event){
  ...
  formData.append("favourite", puuid);
  ...
  if(button.classList.contains("selected")){
      ...
      resp = await fetch("http://127.0.0.1/removeFavourite.php",{
        method: "POST",
        body: formData
      });
  }
      ...
      resp = await fetch("http://127.0.0.1/addFavourite.php",{
        method: "POST",
        body: formData
      });
  }
  res = await resp.text();
  if(res == 0) console.log("operazione andata a buon fine");
  else console.log("errore nell'operazione sulla tabella favourites")
}
const favButton = document.querySelector("#favButton");
favButton.addEventListener('click', addToFavourite);
...
const puuid = document.querySelector('#puuid').textContent;
...
document.querySelector('#profileName #GameName').textContent=gameName;
document.querySelector('#tagLine').textContent='#' + tagLine;


fetch('http://127.0.0.1/summoner_v4.php?puuid=' + puuid).then(onResponse).then(onJsonSummonerv4);
const count = 20;
fetch('http://127.0.0.1/match_v5.php?puuid=' + puuid + '&count=' + count).then(onResponse).then(onJsonMatchv5);